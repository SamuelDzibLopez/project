//APIS

//Usuarios
export const url_usuarios_auth = "/project/server/php/usuarios/auth.php";
export const url_usuarios_validar_correo = "/project/server/php/correos/validar-correo.php";
export const url_usuarios_validar_codigo = "/project/server/php/correos/validar-codigo.php";
export const url_usuarios_cambiar_contraseña = "/project/server/php/correos/cambiar-password.php";
export const url_usuarios_obtener_usuarios =  "/project/server/php/usuarios/obtener-usuarios.php?pagina=";
export const url_usuarios_crear_usuario = "/project/server/php/usuarios/create-user.php";
export const url_usuarios_modificar_usuario = "/project/server/php/usuarios/modificar-usuario.php?idUsuario=";
export const url_usuarios_obtener_info_usuario = "/project/server/php/usuarios/obtener-info-usuario.php?idUsuario=";
export const url_usuarios_validar_usuario = "/project/server/php/usuarios/validar-usuario.php?idUsuario=";
export const url_usurios_eliminar_usuario = "/project/server/php/usuarios/delete.php?idUsuario=";

//Mi perfil
export const url_usuarios_obtener_datos_mi_perfil = "/project/server/php/usuarios/obtener-info-mi-perfil.php"
export const url_usuarios_modificar_datos_mi_perfil = "/project/server/php/usuarios/modificar-mi-perfil.php"

//Procesos
export const url_procesos_obtener_procesos =  "/project/server/php/procesos/obtener-procesos.php?pagina=";

//Contactos
export const url_contactos_obtener_contactos = "/project/server/php/contactos/obtener-contactos.php?pagina=";
export const url_contactos_crear_contacto =  "/project/server/php/contactos/create-contact.php";
export const url_contactos_modificar_contacto =  "/project/server/php/contactos/modificar-contacto.php?idContacto=";
export const url_contactos_obtener_info_contacto =  "/project/server/php/contactos/obtener-info-contacto.php?idContacto=";
export const url_contactos_eliminar_contacto = "/project/server/php/contactos/delete.php?idContacto=";


//URLS

//mi perfil
export const miPerfil = "/project/app/mi-perfil.php";

//usuarios
export const usuarios = "/project/app/usuarios.php";
export const nuevoUsuario = "/project/app/modules/nuevo-usuario.php";

//procesos
export const procesos = "/project/app/procesos.php";
export const nuevoProceso = "/project/app/modules/nuevo-proceso.php";

//documentos
export const documentos = "/project/app/documentos.php";

//contactos
export const directorio = "/project/app/directorio.php";
export const nuevoContacto = "/project/app/modules/nuevo-contacto.php";
